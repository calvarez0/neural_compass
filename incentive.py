...
Explicitly give it a compass. See if it can handle that situation, (ensure it works). 
Figure out how to create a 2-D accelerometer.
Look up "head direction cell" in google scholar. 
...
# Vision parameters
vision_distance = 10.0  # Maximum distance the agent can see
vision_angle = 60.0  # Field of view angle in degrees
...
# Create the vision cone as a wedge
vision_cone = patches.Wedge(agent_position, vision_distance, 0, 0, color='green', alpha=0.2)
...
class QNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(4, 64)  # Input layer: agent position + vector to reward
...
def get_state():
    """Returns the state as a vector of agent's position and vector to the reward."""
    return np.concatenate((agent_position, reward_position - agent_position))

def is_within_vision():
    """Checks if the reward is within the agent's vision range and angle."""
...
def init():
    ...
    return agent_dot, path_trace, reward_dot, vision_line
...
def animate(i):
    ...
    global agent_position, success_flag, gaze_direction, reward_position

    if success_flag:
        # Randomize reward position upon success
    ...
    # Add randomness to the gaze direction to simulate looking around
    gaze_direction += random.uniform(-5.0, 5.0)  # Small random change in direction
...
ani = animation.FuncAnimation(fig, animate, init_func=init, frames=500, interval=100, blit=True)
