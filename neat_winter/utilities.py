...
def randomize_reward_position(width, height, reward_size, beehive_pos, min_distance=25, max_distance=125):
    """Generate reward position within valid distance range from beehive."""
    beehive_x, beehive_y = beehive_pos
    
    # random position until it meets criteria (min_distance <= distance_from_beehive <= max_distance)
    while True:
        reward_x = random.randint(reward_size, width - reward_size)
        reward_y = random.randint(reward_size, height - reward_size)
        
        distance_from_beehive = math.sqrt((reward_x - beehive_x) ** 2 + (reward_y - beehive_y) ** 2)
        
        if min_distance <= distance_from_beehive <= max_distance:
...
def visualize_network_topology(genome, config, generation, run_dir, view=False, filename=None):
    ...
    filepath = os.path.join(run_dir, 'topologies', filename)
...
def plot_stats(statistics, run_dir, ylog=False, view=False, filename='avg_fitness.svg'):
    ...
    filepath = os.path.join(run_dir, 'speciation', filename)
...
def save_agent_simulation(genome, config, generation, run_dir, filename=None):
    ...
    class SimState:
        def __init__(self):
            ...
            # Randomize beehive position with margin
            margin = 10
            self.beehive_pos = (
            ...
            # Make sure this is the same as it is in main if you want sims to look the same as how they are
            # evaluated
            # Use the same randomize_reward_position function as in eval
            self.reward_pos = randomize_reward_position(WIDTH, HEIGHT, REWARD_SIZE, self.beehive_pos, 45, 175)

    state = SimState()
    fig, ax = plt.subplots(figsize=(10, 10))
    
    # Calculate frames for 15 second video at 20 fps
    fps = 20
    max_frames = 15 * fps  # 15 seconds * 20 frames per second = 300 frames


    def animation_frame(frame):
        if state.steps >= max_frames or (state.agent.carrying_reward and state.agent.returned_home):
            return []
        ...
        # Stop animation if agent completes task or simulation is done
        if done or (state.agent.carrying_reward and state.agent.returned_home):
            state.steps = max_frames  # End the animation
            
        return artists
    
    # Create animation with new frame count
    anim = FuncAnimation(fig, animation_frame, frames=max_frames,
...
def create_run_directory():
    ...
    try:
        ...
        subdirs = ['genomes', 'topologies', 'videos', 'speciation', 'config']
