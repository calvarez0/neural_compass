...
def calculate_node_positions(genome, num_inputs, num_outputs, num_hidden):
    ...
    bias_nodes = [n for n in all_nodes if n < 0]
    if bias_nodes:
        bias_spacing = 1.0 / (len(bias_nodes) + 1)
        for i, node_id in enumerate(sorted(bias_nodes)):
            positions[node_id] = np.array([0.1, (i + 1) * bias_spacing * 0.3])  # Scale spacing to use bottom 30%
    
    # Position input nodes above bias nodes
    input_spacing = 1.0 / (num_inputs + 1)
    for i in range(num_inputs):
        positions[i] = np.array([0.1, 0.3 + (i + 1) * input_spacing * 0.7])  # Scale spacing to use upper 70%
    ...
    hidden_layers = sort_hidden_layers(genome, hidden_nodes, num_inputs)
    
    # Position hidden nodes in their respective layers
    if hidden_layers:
...
def sort_hidden_layers(genome, hidden_nodes, num_inputs):
    """Sort hidden nodes into layers based on their connectivity."""
    layers = []
    remaining_nodes = set(hidden_nodes)
    input_nodes = set(range(num_inputs))
    
    while remaining_nodes:
        # Find nodes that only connect to input_nodes or previous layers
        current_layer = set()
        for node in remaining_nodes:
            input_connections = set()
            for conn in genome.connections.values():
                if conn.enabled:
                    # Changed: using key[1] for output node
                    if conn.key[1] == node:
                        # Changed: using key[0] for input node
                        input_connections.add(conn.key[0])
                        
            if input_connections.issubset(input_nodes):
...
def draw_connections(ax, genome, positions, max_weight, cmap):
    ...
    for conn_gene in genome.connections.values():
        if conn_gene.enabled:
            # Changed: using key[0] and key[1] instead of input/output
...
def draw_nodes(ax, genome, positions, num_inputs, num_outputs):
    ...
    for node_id, pos in positions.items():
        ...
        if node_id < 0:  # Any negative ID is a bias node
            color = '#FFB6C1'  # Light pink for bias
            size = 0.03
            label = f'Bias {node_id}'
        elif node_id < num_inputs:
            color = '#AED6F1'  # Light blue for inputs
            size = 0.03
            label = f'In {node_id}'
        elif node_id < num_inputs + num_outputs:
            color = '#A2D9A2'  # Light green for outputs
            size = 0.03
            label = f'Out {node_id}'
        else:
            color = '#F5CBA7'  # Light orange for hidden
            size = 0.025
            label = f'H {node_id}'
