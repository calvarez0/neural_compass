...
class ActivationMonitor:
    def __init__(self, genome, config):
        ...
        self.bars = self.ax.bar(range(self.num_nodes), [0] * self.num_nodes)
        
        # Setup plot
        self.ax.set_ylim(-1, 1)
        self.ax.set_xlim(-0.5, self.num_nodes - 0.5)
        self.ax.set_xlabel('Hidden Node ID')
        self.ax.set_ylabel('Activation')
        self.ax.set_title('Hidden Node Activations')
        
        # Add node ID labels
        plt.xticks(range(self.num_nodes), 
                  [f'Node {n}' for n in self.hidden_nodes], 
                  rotation=45)
        
        # Add horizontal line at y=0
        self.ax.axhline(y=0, color='k', linestyle='-', alpha=0.3)
        
        # Add time window for tracking activation patterns
        self.time_window = 50  # Store last 50 activations
        self.activation_history = {node: [] for node in self.hidden_nodes}
        
        # Create a second subplot for temporal patterns
        self.fig2, self.ax2 = plt.subplots(figsize=(8, 4))
        self.lines = {}
        for node in self.hidden_nodes:
            line, = self.ax2.plot([], [], label=f'Node {node}')
            self.lines[node] = line
        
        self.ax2.set_xlim(0, self.time_window)
        self.ax2.set_ylim(-1, 1)
        self.ax2.set_title('Activation Patterns Over Time')
    ...
    def update_activations(self, activations):
        """Update the bar plot with new activation values."""
        # Update bars
        for idx, node_id in enumerate(self.hidden_nodes):
            if node_id in activations:
                self.bars[idx].set_height(activations[node_id])
                
                # Update activation history
                self.activation_history[node_id].append(activations[node_id])
                if len(self.activation_history[node_id]) > self.time_window:
                    self.activation_history[node_id].pop(0)
        
        # Update temporal pattern plot
        for node_id, line in self.lines.items():
...
class ModifiedFeedForwardNetwork(neat.nn.FeedForwardNetwork):
    """Modified NEAT network that exposes node activations."""
    @classmethod
    def create(cls, genome, config):
        """Create a new FeedForwardNetwork."""
        net = super().create(genome, config)
        # Convert the created network to our modified class
        net.__class__ = cls
        return net
    
    def activate(self, inputs):
        """Get network output and save node activations."""
...
def create_network_with_monitoring(genome, config):
    """Create a network with activation monitoring capabilities."""
    return ModifiedFeedForwardNetwork.create(genome, config)
...
def visualize_agent(genome, config, max_steps=None):
    """
    Visualize agent behavior with neural network activation monitoring
    """
    # Create network with activation monitoring
    net = create_network_with_monitoring(genome, config)
    monitor = ActivationMonitor(genome, config)
    ...
    def update():
        ...
        # Update activation monitor
        monitor.update_activations(net.hidden_activations)
...
def save_agent_simulation(genome, config, generation, run_dir, max_steps=500, filename=None):
    ...
    # Rest of your existing save_agent_simulation code here, just use filepath instead
    # Create network with monitoring at the start of the function
    net = create_network_with_monitoring(genome, config)
...
def visualize_saved_genome(filepath: str, max_steps: int = 1000):
    """
    Load and visualize a saved genome.
    
    Args:
        filepath: Path to the saved genome file
        max_steps: Maximum number of simulation steps
    """
    # Load the saved genome and config
    genome, config, generation = load_genome(filepath)
    
    # Visualize the agent's behavior with genome directly
    visualize_agent(genome, config, max_steps=max_steps)
    
    # Optionally visualize the network topology
    visualize_network_topology(genome, config, generation)
